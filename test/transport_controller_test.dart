...
void main() {
  ...
  test(
    'realtime reconnect and changed signals reload both canonical lists',
    () async {
      await driversController.saveDriver(
        const DriverInput(fullName: 'Before'),
        requestId: 'one',
      );
      final row = driversController.state.drivers.single;
      repository.driversStore[row.id] = row.copyWith(
        fullName: 'Remote',
        version: 2,
      );
  ...
  test(
    'delete and restore preserve operational data for both entities',
    () async {
      await driversController.saveDriver(
        const DriverInput(
          fullName: 'Driver',
          whatsappPhone: '+972123',
      ...
      expect(
        await driversController.restoreDriver(
          repository.driversStore[driver.id]!,
        ),
        true,
      );
      expect(driversController.state.drivers.single.whatsappPhone, '+972123');
      expect(
        driversController.state.drivers.single.status,
      ...
      expect(
        await vehiclesController.restoreVehicle(
          repository.vehiclesStore[vehicle.id]!,
        ),
        true,
      );
  ...
  test(
    'CAS conflicts reconcile and remain explicit through periodic reads',
    () async {
      await driversController.saveDriver(
        const DriverInput(fullName: 'Before'),
        requestId: 'one',
      ...
      repository.driversStore[base.id] = base.copyWith(
        fullName: 'Remote',
        version: 2,
      );
      expect(
        await driversController.saveDriver(
          const DriverInput(fullName: 'Draft'),
          requestId: 'edit',
          base: base,
        ),
        false,
  ...
  test(
    'connectivity and authorization failures are distinct and disable writes',
    () async {
      ...
      await driversController.refresh();
      await driversController.saveDriver(
        const DriverInput(fullName: 'Private'),
        requestId: 'one',
      );
  ...
  test(
    'revocation during a read cannot repopulate data or selection',
    () async {
  ...
  test(
    'invalidation during fetch reruns and close cancels subscriptions',
    () async {
      final gate = Completer<void>();
      repository.readGate = gate.future;
  ...
  test('periodic reconciliation runs without realtime notifications', () async {
    ...
    final controller = DriversController(
      repo,
      events,
