...
class VehiclesController extends Cubit<VehiclesState> {
  ...
  VehiclesController.forEvent(
    ...
    if (eventId != repository.eventId) {
      throw ArgumentError('Transport event mismatch');
    }
  }
  ...
  void _set({
    ...
    emit(
      state.copyWith(
        ...
        clearSelectedVehicle: clear || clearSelected,
        synchronizedAt: at,
      ),
    );
  }
  ...
  Future<void> _readLoop() async {
    do {
      ...
      try {
        final rows = await repository.listVehicles(
          _eventId,
          query: state.query,
        ...
        final selected = selectedId == null
            ? null
            : await repository.readVehicle(_eventId, selectedId);
        if (_stopped || generation != _generation || !_access) continue;
        _set(
          vehicles: List.unmodifiable(rows),
          online: true,
          selected: selected,
  ...
  Future<bool> saveVehicle(
    ...
    final id = await repository.saveVehicle(
      _eventId,
      input,
      requestId: requestId,
      id: base?.id,
  ...
  Future<bool> deleteVehicle(Vehicle base) => _mutate(
  ...
  Future<bool> restoreVehicle(Vehicle base) => _mutate(
    () => repository.restoreVehicle(
      _eventId,
      base.id,
      expectedVersion: base.version,
    ),
  ...
  void beginEdit() {
    if (!_stopped && state.save != SaveStatus.saving) {
      emit(state.copyWith(save: SaveStatus.idle));
    }
  }
