...
class DriversController extends Cubit<DriversState> {
  ...
  void _set({
    ...
    emit(
      state.copyWith(
        ...
        clearSelectedDriver: clear || clearSelected,
        synchronizedAt: at,
      ),
    );
  }
  ...
  Future<void> _readLoop() async {
    do {
      ...
      try {
        final rows = await repository.listDrivers(
          _eventId,
          query: state.query,
        ...
        final selected = selectedId == null
            ? null
            : await repository.readDriver(_eventId, selectedId);
        if (_stopped || generation != _generation || !_access) continue;
        _set(
          drivers: List.unmodifiable(rows),
          online: true,
          selected: selected,
  ...
  Future<bool> saveDriver(
    ...
    final id = await repository.saveDriver(
      _eventId,
      input,
      requestId: requestId,
      id: base?.id,
  ...
  Future<bool> deleteDriver(Driver base) => _mutate(
  ...
  Future<bool> restoreDriver(Driver base) => _mutate(
    () => repository.restoreDriver(
      _eventId,
      base.id,
      expectedVersion: base.version,
    ),
